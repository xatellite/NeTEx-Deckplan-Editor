<template>
  <div class="flex gap-4 h-full overflow-hidden">
    <!-- Configuration Panel -->
    <div class="flex-1 flex flex-col min-h-0 bg-ott-bg-primary border-r border-ott-bg-dark">
    ...
    <div class="w-64 flex flex-col gap-2">
      ...
      <div class="border-t border-ott-bg-dark mt-2 pt-2 flex-1 overflow-y-auto px-2 pb-4">
        <div class="flex flex-col gap-2">
          <div
            v-for="item in filteredElements"
            :key="item.id"
            @click="() => createNewElement(item)"
            :class="`p-3 rounded-lg border flex items-center gap-3 transition-all cursor-pointer hover:bg-ott-bg-secondary group ${elementToBuild?.constructor === item.model ? 'border-ott-accent bg-ott-accent/5' : 'border-ott-bg-dark'}`"
          >
...
const categories = [
  { id: 'seat', icon: 'material-symbols:event-seat-outline-rounded' },
  { id: 'luggage', icon: 'material-symbols:luggage-outline-rounded' },
  { id: 'door', icon: 'material-symbols:door-open-outline-rounded' },
  { id: 'space', icon: 'fluent:breakout-room-32-regular' }
];

const elements = [
  { id: 'seat', category: 'seat', label: 'Passenger Seat', icon: 'material-symbols:event-seat-outline-rounded', model: PassengerSpot },
  { id: 'luggage-spot', category: 'luggage', label: 'Luggage Spot', icon: 'material-symbols:luggage-outline-rounded', model: LuggageSpot },
  ...
  { id: 'toilet', category: 'space', label: 'Toilet', icon: 'material-symbols:toilet-outline-rounded', model: PassengerSpace, type: 'toilet' },
  { id: 'corridor', category: 'space', label: 'Corridor', icon: 'material-symbols:straight-outline-rounded', model: PassengerSpace, type: 'corridor' }
];

const enabledCategories: Ref<Set<any>> = ref(new Set(categories));
